...
double eigenFreq::getGeomMean() {
    return (alpha - 0.5) / (alpha + beta - 0.5) * upper_freq;
};
...
void eigenFreq::update(int sample) {
    ...
    if (iteration > 2) {
        ...
        for (std::vector<double>::size_type j = 1; j < b.size(); ++j) {
...
double eigenFreq::getEstimatedFreq() {
    if (iteration < 3) {
        ...
        return filtered[iteration - 1] * upper_freq;
    }
};
...
double eigenFreq::iBeta(double a, double b, double x) {
    if (x < 0.0 || x > 1.0) throw std::out_of_range("x must be between 0 and 1");
    ...
    throw std::runtime_error("iBeta did not converge after 200 iterations");
}
